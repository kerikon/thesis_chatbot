"""
SOURCE: https://github.com/JiteshGaikwad/EntityExtractor, last accessed 05. August 2020
"""
...
class MSRTExtractor(Component):
    ...
    def convert_to_rasa(
        ...
        for match in matches:
            entity = {"start": match["start"], "end": match["end"], "text": match["text"], "entity": match["type_name"]}
            ...
            if "values" in match["resolution"]:
                ...
                if match["type_name"] == "datetimeV2.datetime":
                    ...
                    if "start" not in match["resolution"]["values"][-1] and \
                            "end" not in match["resolution"]["values"][-1]:
    ...
    def _parse_all_entities(user_input: str,
                            culture: str) -> List[List[ModelResult]]:
        """
    ...
    @staticmethod
    def _parse_entities(self, text: Text, language: str) -> List[Dict[Text, Any]]:
        """pass the user input to the recognizer model to parse the entities.
            
            Required Parameter: 
            @user_input -> user entered text,
            
            @language -> language for prediction
            """

        try:
            result = self._parse_all_entities(text, language)
            # Flatten results
            result = [item for sublist in result for item in sublist]
            result = json.dumps(result, default=lambda o: o.__dict__, indent='\t', ensure_ascii=False)
            return self.convert_to_rasa(json.loads(result))
    ...
    def process(self, message, **kwargs):
        ...
        extracted_entities = self._parse_entities(self, message.text, language)
